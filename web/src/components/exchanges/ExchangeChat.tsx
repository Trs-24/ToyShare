    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div
                ...
                className="p-6 max-h-[400px] overflow-y-auto space-y-4" style={{ minHeight: '200px' }}>
                {messages.length === 0 ? (
                    ...
                    messages.map(msg => {
                        ...
                        return (
                            ...
                            >
                                {/* Avatar */}
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold ${isMine
                                ...
                                <div className={`max-w-[70%] ${isMine ? 'text-right' : ''}`}>
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className={`text-xs font-medium ${isMine ? 'text-indigo-600' : 'text-gray-700'}`}>
                                        ...
                                        {isEdited && (
                                            <span className="text-xs text-gray-400 italic">{t('exDetail_chatEdited')}</span>
                                        )}
                                    </div>
                                    ...
                                    {editingId === msg.id ? (
                                        ...
                                        <div className="flex gap-2">
                                            <input
                                                ...
                                                className="flex-1 px-3 py-1.5 rounded-lg border border-indigo-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none text-sm"
                                                autoFocus
                                            />
                                            <button
                                        ...
                                        <div className="relative inline-block text-left">
                                            ...
                                            {isMine && exchangeStatus !== 'COMPLETED' && (
                                                <div className={`absolute top-0 ${isMine ? 'left-0 -translate-x-full' : 'right-0 translate-x-full'} flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity px-1`}>
                                                    <button
                                                        onClick={() => {
                                                            setEditingId(msg.id);
                ...
                <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/30">
                    <div className="flex gap-3">
                        <input
                            ...
                            className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none transition-all text-sm disabled:opacity-50"
                        />
                        <button
                            onClick={handleSend}
                            disabled={sendingMessage || !newMessage.trim()}
                            className="px-5 py-2.5 rounded-xl bg-indigo-600 text-white font-medium hover:bg-indigo-700 shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                        >
                            {sendingMessage ? (
