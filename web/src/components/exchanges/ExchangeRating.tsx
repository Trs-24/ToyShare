...
export default function ExchangeRating({
    myRating,
    otherRating,
    onSubmit
}: ExchangeRatingProps) {
    ...
    const StarRating = ({ value, interactive = false }: { value: number; interactive?: boolean }) => (
        <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map(star => (
                ...
                >
                    <span className={star <= (interactive ? (ratingHover || value) : value) ? 'text-amber-400' : 'text-gray-300'}>
                        ★
                    </span>
                </button>
            ))}
            ...
            {myRating ? (
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows={2}
                        className="w-full mt-3 px-4 py-2.5 rounded-xl border border-gray-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 outline-none transition-all text-sm resize-none"
                    />
                    <button
