...
export default function ExchangeShipping({
  ...
  const [meetingDate, setMeetingDate] = useState(
    exchange.meetingDate ? exchange.meetingDate.slice(0, 10) : '',
  );
  ...
  const myShippingConfirmed = isInitiator
  ...
  return (
    <div className="mb-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
      <h2 className="text-base font-bold text-gray-900 mb-4 flex items-center gap-2">
        ...
        {myShippingConfirmed && (
          <span className="ml-2 px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-xs font-semibold">
      ...
      {myShippingConfirmed ? (
        ...
        <div className="bg-emerald-50/50 rounded-xl p-4 border border-emerald-100">
          ...
          <div className="grid sm:grid-cols-2 gap-3 text-sm">
            {exchange.meetingDate && (
              <div>
                <span className="text-gray-500">{t('exDetail_shippingDate')}:</span>{' '}
                <span className="font-medium text-gray-900">
                  {new Date(exchange.meetingDate).toLocaleDateString(dateLocale)}
                </span>
              </div>
            )}
            {exchange.postOffice && (
              <div>
                <span className="text-gray-500">{t('exDetail_shippingPostOffice')}:</span>{' '}
                <span className="font-medium text-gray-900">{exchange.postOffice}</span>
              </div>
            )}
          </div>
          {exchange.shippingNote && (
            <p className="text-sm text-gray-600 mt-2">
              <span className="text-gray-500">{t('exDetail_shippingNote')}:</span>{' '}
              {exchange.shippingNote}
            </p>
          )}
        </div>
        ...
        <>
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-teal-400 focus:ring-2 focus:ring-teal-100 outline-none transition-all text-sm resize-none"
            />
          </div>
          <button
            onClick={handleSave}
          ...
          >
            ...
            {exchange.status === 'ACCEPTED'
              ? t('exDetail_shippingGoToShipping')
              : t('exDetail_shippingSave')}
          </button>
          {exchange.status === 'ACCEPTED' && (
            <p className="text-xs text-gray-400 mt-2">{t('exDetail_shippingStatusNote')}</p>
          )}
        </>
      ) : (
        ...
        <>
          ...
          <p className="text-xs text-amber-600 mt-2">⚠️ {t('shipping_confirmWarning')}</p>
        </>
      )}
    </div>
  );
