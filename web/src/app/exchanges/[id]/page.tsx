...
export default function ExchangeDetailPage() {
    ...
    const handleEditMessage = async (msgId: string, content: string) => {
        ...
        setMessages(prev =>
            prev.map(m => m.id === msgId ? { ...m, content: updated.content, updatedAt: updated.updatedAt } : m)
        );
    };
            ...
            <div className="max-w-3xl mx-auto p-6 pb-12">
                ...
                <div className="grid md:grid-cols-2 gap-8 mb-8">
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        <div className="aspect-square bg-gray-100 rounded-xl mb-4 overflow-hidden">
                            {theirItem?.photos?.[0] ? (
                                <img src={getMediaUrl(theirItem.photos[0].url)} className="w-full h-full object-cover" />
                            ) : (
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        <div className="aspect-square bg-gray-100 rounded-xl mb-4 overflow-hidden">
                            {myItem?.photos?.[0] ? (
                                <img src={getMediaUrl(myItem.photos[0].url)} className="w-full h-full object-cover" />
                            ) : (
                                <div className="w-full h-full flex items-center justify-center text-4xl">🧸</div>
                            )}
                        </div>
                ...
                <div className="flex justify-end gap-4 mb-8">
                    ...
                    {exchange.status === 'IN_PROGRESS' && (() => {
                        ...
                        const myConfirmed = isInitiator ? exchange.initiatorCompleted : exchange.receiverCompleted;
                        const otherConfirmed = isInitiator ? exchange.receiverCompleted : exchange.initiatorCompleted;
                        const myShippingOk = isInitiator ? exchange.initiatorShippingConfirmed : exchange.receiverShippingConfirmed;
                        const otherShippingOk = isInitiator ? exchange.receiverShippingConfirmed : exchange.initiatorShippingConfirmed;
                        const bothShippingConfirmed = myShippingOk && otherShippingOk;

                        return (
                            <div className="w-full">
                                <div className="px-6 py-2.5 rounded-xl bg-purple-50 text-purple-700 font-medium mb-4">
                                    {t('exDetail_inProgress')}
                                </div>
                                <div className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
                                    <p className="text-sm font-semibold text-gray-700 mb-3">{t('exDetail_completionConfirm')}</p>

                                    {/* Shipping confirmation status */}
                                    <div className="flex flex-col gap-2 mb-3">
                                        <div className="flex items-center gap-2 text-sm">
                                            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${myShippingOk ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400'}`}>
                                                {myShippingOk ? '📦' : '·'}
                                            </span>
                                            <span className={myShippingOk ? 'text-blue-700 font-medium' : 'text-gray-500'}>
                                                {t('you')}: {myShippingOk ? t('shipping_confirmed') : t('shipping_needConfirm')}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-2 text-sm">
                                            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${otherShippingOk ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400'}`}>
                                                {otherShippingOk ? '📦' : '·'}
                                            </span>
                                            <span className={otherShippingOk ? 'text-blue-700 font-medium' : 'text-gray-500'}>
                                                {otherUser?.name || t('exDetail_participant')}: {otherShippingOk ? t('shipping_confirmed') : t('shipping_needConfirm')}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-2 mb-4">
                                        <div className="flex items-center gap-2 text-sm">
                                            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${myConfirmed ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-100 text-gray-400'}`}>
                                                {myConfirmed ? '✓' : '·'}
                                            </span>
                                            <span className={myConfirmed ? 'text-emerald-700 font-medium' : 'text-gray-500'}>
                                                {t('you')} {myConfirmed ? t('exDetail_youConfirmed') : t('exDetail_youNotConfirmed')}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-2 text-sm">
                                            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${otherConfirmed ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-100 text-gray-400'}`}>
                                                {otherConfirmed ? '✓' : '·'}
                                            </span>
                                            <span className={otherConfirmed ? 'text-emerald-700 font-medium' : 'text-gray-500'}>
                                                {otherUser?.name || t('exDetail_participant')} {otherConfirmed ? t('exDetail_otherConfirmed') : t('exDetail_otherNotConfirmed')}
                                            </span>
                                        </div>
                                    </div>
                                    {!myConfirmed ? (
                ...
                {isActive && otherUser && (
                    <ExchangeContact user={otherUser} />
                )}
