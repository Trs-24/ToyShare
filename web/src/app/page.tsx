            ...
            <main className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className="flex flex-col md:flex-row gap-8">
                    ...
                    <aside className="w-full md:w-64 space-y-5 flex-shrink-0">
                        ...
                        {/* City Filter */}
                        <div className="pt-2">
                            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('catalog_city')}</label>
                            <input
                                type="text"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('catalog_category')}</label>
                            <select
                                className="input-field"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('catalog_condition')}</label>
                            <select
                                className="input-field"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('catalog_gender')}</label>
                            <select
                                className="input-field"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('catalog_age')}</label>
                            <select
                                className="input-field"
                                value={filters.age}
                            ...
                            >
                                ...
                                {ageOptions.filter(o => o.value).map((opt) => (
                                    <option key={opt.value} value={opt.value}>{opt.label}</option>
                                ))}
                            </select>
                        </div>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('catalog_type')}</label>
                            <select
                                className="input-field"
                                value={filters.type}
                                    ...
                                    <div className="flex rounded-lg border border-gray-200 overflow-hidden">
                                        {PER_PAGE_OPTIONS.map((opt) => (
                                            <button
                                                ...
                                                className={`px-3 py-1.5 text-sm font-medium transition-colors ${limit === opt
                        ...
                        {loading ? (
                            ...
                            <>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {items.map((item) => (
                                        <Link key={item.id} href={`/items/${item.id}`} className="card group hover:shadow-lg transition">
                                            <div className="aspect-square bg-gray-100 relative overflow-hidden rounded-t-xl">
                                                ...
                                                <div className="absolute top-2 right-2 flex gap-1">
                                                    {item.condition && <span className="badge bg-white/90 text-gray-700 shadow-sm text-xs">{getConditionLabel(t, item.condition)}</span>}
                                                </div>
